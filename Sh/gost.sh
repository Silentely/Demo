...
download_gost(){
	...
	mkdir /root/.gost
	wget --no-check-certificate https://raw.githubusercontent.com/Silentely/Demo/main/Sh/config.json.example -O /root/.gost/config.json
	echo -e "${Info} gost 主程序安装完毕！开始配置服务文件..."
}

service_gost(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/Silentely/Demo/main/Sh/gost_centos.service -O /etc/init.d/gost; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/Silentely/Demo/main/Sh/gost_debian.service -O /etc/init.d/gost; then
			echo -e "${Error} gost服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/gost
...
config_gost_f_add(){
	echo -e "请输入 -F 参数"
	read -e -p "(默认 - [http://192.168.1.1:8080] 例如"http://XX.XX.XX.XX:XXXX",如需设置密码则输入“http://admin:123456@192.168.1.1:8080”): " param_f
