...
if [ -z "$CLOUDFLARE_IPS_V4" ]; then
    ...
    for ip in $CLOUDFLARE_IPS_V4; do
        echo "  - 允许 IPv4: $ip (端口 80/tcp 和 443/tcp)"
        # 分别添加 HTTP (80) 和 HTTPS (443) 的 TCP 规则
        sudo ufw allow from "$ip" to any port 80 proto tcp || echo "警告:添加 IPv4 规则失败:$ip:80"
        sudo ufw allow from "$ip" to any port 443 proto tcp || echo "警告:添加 IPv4 规则失败:$ip:443"
    done
...
if [ -z "$CLOUDFLARE_IPS_V6" ]; then
    ...
    for ip in $CLOUDFLARE_IPS_V6; do
        echo "  - 允许 IPv6: $ip (端口 80/tcp 和 443/tcp)"
        # 分别添加 HTTP (80) 和 HTTPS (443) 的 TCP 规则
        sudo ufw allow from "$ip" to any port 80 proto tcp || echo "警告:添加 IPv6 规则失败:$ip:80"
        sudo ufw allow from "$ip" to any port 443 proto tcp || echo "警告:添加 IPv6 规则失败:$ip:443"
    done
